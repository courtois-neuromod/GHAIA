...
# ---------------------------------------------------------------------------
# 8. Write the HPC environment file
# ---------------------------------------------------------------------------
# Written here — right after the venv is built — so it exists even if a later
# data or smoke-test step fails. Job scripts source this file instead of
# hardcoding modules/paths or requiring shell-profile edits; the fixed
# location lets any script find it regardless of where it is submitted from.
...
# ---------------------------------------------------------------------------
# 10. Subject data check
# ---------------------------------------------------------------------------
# `find -print -quit` stops after the first hit (no SIGPIPE), and the whole
# test runs as an `if` condition so a missing dir or no matches never aborts
# the script under `set -euo pipefail`.
SCENES_DIR="$MARIHA_DATA_ROOT/mario.scenes"
if [[ -d "$SCENES_DIR" ]] && find "$SCENES_DIR" -name "*.state" -print -quit 2>/dev/null | grep -q .; then
...
# ---------------------------------------------------------------------------
# 11. Generate per-scene scenario files
# ---------------------------------------------------------------------------
...
# 12. Smoke test
# ---------------------------------------------------------------------------
# Non-fatal: the env file is already written, so a failed import here should
# report the problem without aborting the whole setup.
...
then
  ...
  warn "Smoke test failed — env file is still in place; fix the imports and rerun."
fi
